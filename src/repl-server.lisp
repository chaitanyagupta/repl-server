...
(defun rep (&aux (*standard-output* (two-way-stream-output-stream *repl-stream*)))
  ...
  (let ((to-eval (handler-case (read-js *repl-stream*)
    ...
    (let* ((response-string (eval-string to-eval))
           ...
           (result-str (make-array '(0) :element-type 'base-char
                             :fill-pointer 0 :adjustable t)))
      (with-repl-stream-lock
        (with-output-to-string (result-stream result-str)
          (cond
            ((string-equal type "Undefined")
             (princ "undefined" result-stream))
            ((string-equal type "Null")
             (princ "null" result-stream))
            ((string-equal type "Boolean")
             (if value (princ "true" result-stream) (princ "false" result-stream)))
            ((string-equal type "Number")
             (princ value result-stream))
            ((string-equal type "String")
             (prin1 value result-stream))
            ((string-equal type "RegExp")
             (princ value result-stream))
            ((string-equal type "Array")
             (json:encode-json value result-stream))
            ((string-equal type "Function")
             (let ((source (dot response :source))
                   (name (dot response :name)))
               (if source
                   (princ source result-stream)
                   (progn
                     (princ "<function>" result-stream)
                     (format result-stream " ~A" name)))))
            ((string-equal type "Error")
             (if (dot response :thrown)
                 (progn
                   ...
                   (reset))
                 (format result-stream "<Error> Type: ~A~%~A" constructor (dot response :error))))
            ((string-equal type "Object")
             (princ "<object>" result-stream)
             (when constructor
               (format result-stream "/[~A]" constructor)))
            (t
             (apply-color :error)
             (format t "Unrecognized object:~%~S" response-string)
             (reset)))
          (when (plusp (length result-str))
            (apply-color :result)
            (princ result-str)
            (reset)))
        (terpri)))))
